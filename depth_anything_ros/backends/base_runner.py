...
class BaseRunner:
    def __init__(self, model_path: str, input_size_w: int = 0, input_size_h: int = 0):
        self.model_path = model_path
        self.input_size_w = input_size_w  # 0 = use original width
        self.input_size_h = input_size_h  # 0 = use original height

    def preprocess_resize(self, cv_image):
        """Resize image if input_size is set. Returns (resized_image, original_size)"""
        orig_h, orig_w = cv_image.shape[:2]
        if self.input_size_w > 0 and self.input_size_h > 0:
            resized = cv2.resize(cv_image, (self.input_size_w, self.input_size_h), interpolation=cv2.INTER_LINEAR)
